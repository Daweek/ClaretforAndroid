...
void Reconfig(){
  ...
  sub_x = 1.5;
 	sub_y = 1.5;
  temp_ymax = 2000;
  for(i = 0; i < DATA_NUM; i++)temp_data[i] = 0;
...
float appRender(JNIEnv* env, jobject obj,int width,int height, int dscuforce)
{
    ...
    gflops = (float)n1*(float)n1*78/(md_time-md_time0)*1e-9;
    //LOGI("Gflops: %f ",gflops);
